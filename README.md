...
AI·데이터 문제해결은행의 개방데이터와 직접 확보한 적법한 데이터를 활용해, 상권·업종 단위의 소상공인 폐업위험을 조기에 포착할 수 있는 신호와 지표를 설계하는 프로젝트입니다. 결과물은 **2026 데이터+AI 혁신 챌린지 데이터 문제해결 부문 - 데이터레시피 제안** 유형에 제출합니다.
...
- 제출물: 참가신청서, 포털에서 작성한 데이터레시피, 본선 진출 시 발표자료
- 예선 핵심 기준: 기획성 25, 구체성 25, 실효성 25, 종합 완성도 25
- 제약: 분석도구 활용 유형과 중복 참가 불가, 데이터 직접 확보, 개인정보 비식별화 및 저작권 확인 필수

세부 요건, 평가표, 일정과 원문 간 주의사항은 [공모 요건 정리](docs/kor/project/COMPETITION_REQUIREMENTS.md)를 참고하세요. 공고 원본 PDF와 파일 무결성 정보는 [공모 원문 자료](docs/references/README.md)에 보관합니다.
...
4. 최대 6단계로 재현 가능한 분석 프로세스와 워크스튜디오 지원 분석 결과
...
| 02 | 2026-09-23 ~ 2026-09-29 | 문제정의, 레시피 초안, 6단계 분석 설계 | COD-192~194 |
...
Sprint별 완료 조건과 리스크는 [프로젝트 로드맵](docs/kor/project/PROJECT_ROADMAP.md)에 정리합니다.
...
- 데이터레시피 제안 유형으로 범위 전환, 데이터 후보와 타깃 정의 검토 진행 중
...
- [문서 안내](docs/kor/README.md)
- [공식 자료·가이드 검토 노트](docs/kor/research/GUIDE_NOTES.md)
- [작업 가이드](docs/kor/project/WORK_GUIDE.md)
...
- [공모 요건](docs/kor/project/COMPETITION_REQUIREMENTS.md)
- [프로젝트 로드맵](docs/kor/project/PROJECT_ROADMAP.md)
- [Git 워크플로](docs/kor/project/GIT_WORKFLOW.md)
...
| 경로 | 역할 |
| --- | --- |
| `src/sme_closure_signal/` | 검증·데이터셋 생성·분석 함수와 CLI, 점검용 모듈 |
| `pyproject.toml` | 패키지·의존성·CLI 설정 |
| `data/` | 로컬 원천·중간·모델 입력 데이터; Git 제외 |
| `outputs/` | 검증·모델·그림 생성물; Git 제외 |
| `docs/kor/project/` | 공모 요건, 일정과 작업 규칙 |
| `docs/kor/research/` | 가이드 조사, 가설과 실제 데이터 검증 |
| `docs/kor/recipe/` | 문제 해결레시피와 코드 기반 분석 설계 |

개발 환경에서는 `python -m pip install -e .`로 패키지를 설치한 뒤 `python -m sme_closure_signal --help`에서 시작합니다. 파일 배치 규칙은 [데이터 안내](data/README.md), [출력 안내](outputs/README.md), [핵심 코드 안내](src/README.md)를 따릅니다.
