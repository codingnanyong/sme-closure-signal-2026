...
| [COD-194](https://linear.app/codingnanyong/issue/COD-194) | 워크스튜디오 워크플로 설계 | 단계, 입출력, 모델 후보 설계 |

단계 완료 조건:

- 측정 가능한 타깃과 기준 시점 정의
- 가설별 필요한 변수와 검증 방법 매핑
- 워크스튜디오에서 구현 가능한 분석 범위 확인
...
| [COD-197](https://linear.app/codingnanyong/issue/COD-197) | 워크스튜디오 1차 실행 | 실행 가능한 워크플로와 1차 결과 |

단계 완료 조건:

- 시간 누수 없는 학습·검증 분할
- 단순 기준선과 후보 모델 비교
- 같은 입력으로 재실행 가능한 워크플로 저장
...
| 리스크 | 영향 | 대응 |
| --- | --- | --- |
| 분석도구 활용 부문과 주제의 적합성 | 참가 부적격 가능성 | COD-189에서 공식 문의로 확인하고 사회현안 연결을 명시 |
...
| 워크스튜디오 기능 제약 | 설계 재작업 | Sprint 02에 최소 워크플로를 먼저 실행 |
...
- 워크스튜디오에서 동일 분석을 재실행할 수 있다.
- 기준모델 대비 결과와 세부집단 검증을 제시했다.
- 데이터레시피, JSON, 워크플로 URL, 신청서를 준비했다.
