...
### Sprint 03 시작 준비 (`2026-09-18`)

- 로컬 데이터는 `raw / interim / processed / outputs`로 배치하고 체크섬을 기록했다.
- 미리보기 길단위인구의 완전 동일 중복 82,710행을 제거해 8개 분기 13,190개 키로 정리했다.
- 전체 파이프라인을 재실행해 모델 입력 611,664행과 타깃 가용 531,670행을 확인했다.
- 공식 길단위인구 API 키와 전 기간 공식 수집은 COD-195의 첫 게이트로 남긴다.
- 상세 시작 절차는 [Sprint 03 시작 준비](SPRINT_03_STARTUP.md)를 따른다.
